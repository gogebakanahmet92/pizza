...
def get_all_tasks():
    ...
    data2.current_user_id = User.query.filter_by(
        name=data2.current_user).first().id
...
def add_new_task():
    ...
    new_task = Tasks(task_path=pizza_string,
                     user_id=data2.current_user_id, task_name=data2.current_task)
    new_task.save_to_db()
    return jsonify({'message': 'File!', 'task': data2.task_dict})
